...
pub fn render_markdown(env: &Envelope) -> String {
    ...
    render_markdown_features(&mut b, &env.serp_features);
    b
}

fn render_markdown_features(b: &mut String, features: &[crate::core::types::SerpFeature]) {
...
pub fn render_text(env: &Envelope) -> String {
    ...
    render_text_features(&mut b, &env.serp_features);
    b
}

fn render_text_features(b: &mut String, features: &[crate::core::types::SerpFeature]) {
    if features.is_empty() {
        return;
    }
    b.push_str("Features\n\n");
