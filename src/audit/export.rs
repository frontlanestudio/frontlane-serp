...
impl KeywordAuditReport {
    ...
    pub fn to_csv(&self) -> String {
        ...
        for page in all_pages {
            ...
            let rating_str = page.rating_value.map(|r| format!("{:.1}", r)).unwrap_or_default();
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str(&format!("- **Opportunity Score**: **{}/100**\n", self.opportunity_score));
        ...
        out.push_str("| Rank | Domain | Type | Title | H1 | Words | Exact KW | Schema | Rating |\n");
        out.push_str("| :---: | :--- | :---: | :--- | :--- | :---: | :---: | :---: | :---: |\n");
        ...
        if let Some(ref t) = self.target_audit {
            ...
            out.push_str(&format!(
                ...
                if t.has_local_business_schema { "Local" } else { "—" },
                rating_cell
            ));
        }
        ...
        out.push_str(&format!("- **Directory Aggregator Ratio**: {:.0}%\n", self.benchmarks.directory_aggregator_pct));
        out.push_str(&format!("- **Structured Schema Adoption**: {:.0}% (FAQ Schema: {:.0}%)\n",
            self.benchmarks.schema_adoption_pct, self.benchmarks.faq_schema_adoption_pct));
        out.push_str(&format!("- **Avg Call/Conversion Links**: {:.1} phone links | {:.1} lead forms\n\n",
    ...
    pub fn to_console_text(&self) -> String {
        ...
        out.push_str(&format!("   Target: {} | Engine: {} | Current Rank: {} | Opportunity Score: {}/100\n\n",
        ...
        let header = format!("{:<8} | {:<22} | {:<10} | {:<30} | {:<6} | {:<8} | {}", "Rank", "Domain", "Type", "Title", "Words", "Exact KW", "Schema");
        out.push_str(&header);
        out.push('\n');
        ...
        for comp in &self.competitor_audits {
            out.push_str(&format!("{:<8} | {:<22} | {:<10} | {:<30} | {:<6} | {:<8} | {}\n",
                ...
                comp.word_count,
                comp.exact_keyword_count,
                if comp.has_local_business_schema { "Local" } else { "—" }
            ));
        }
        ...
        out.push_str(&format!("   • Directory Ratio: {:.0}%\n", self.benchmarks.directory_aggregator_pct));
