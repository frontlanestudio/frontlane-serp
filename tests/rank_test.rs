use frontlane_serp::rank::{
    calculate_pages_to_probe, matches_target, normalize_domain_or_url, DomainMatchMode,
    RankStrategy,
};
...
fn test_matches_target_modes() {
    ...
    assert!(!matches_target(
        "https://sub.example.com/services",
        "example.com/services",
        DomainMatchMode::Exact
    ));
...
fn test_smart_probing_math() {
    ...
    assert_eq!(
        calculate_pages_to_probe(RankStrategy::Smart, 1, 5),
        vec![1, 2]
    );

    // Rank 15 -> Page 2 -> neighbors [1, 2, 3]
    assert_eq!(
        calculate_pages_to_probe(RankStrategy::Smart, 15, 5),
        vec![1, 2, 3]
    );

    // Rank 25 -> Page 3 -> neighbors [2, 3, 4]
    assert_eq!(
        calculate_pages_to_probe(RankStrategy::Smart, 25, 5),
        vec![2, 3, 4]
    );
    ...
    assert_eq!(
        calculate_pages_to_probe(RankStrategy::Basic, 45, 5),
        vec![1]
    );

    // Custom strategy -> 1..=limit
    assert_eq!(
        calculate_pages_to_probe(RankStrategy::Custom, 45, 4),
        vec![1, 2, 3, 4]
    );
}
